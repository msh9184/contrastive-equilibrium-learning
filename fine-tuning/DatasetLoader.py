...
def loadWAV(filename, max_frames, evalmode=True, num_eval=10):
    ...
    if evalmode:
        torchfb        = transforms.MelSpectrogram(sample_rate=16000, n_fft=512, win_length=400, hop_length=160, f_min=0.0, f_max=8000, pad=0, n_mels=40);
        instancenorm   = nn.InstanceNorm1d(40);
        feat = torchfb(feat)+1e-6
        feat = instancenorm(feat.log()).detach()
...
class DatasetLoader(object):
    ...
    def dataLoaderThread(self, nThreadIndex):
        ...
        while(True):
            ...
            in_fb = []
            for data in in_data:
                data = self.torchfb(data)+1e-6
                in_fb.append(self.instancenorm(data.log()))

            in_label = numpy.asarray(self.data_label[index:index+self.batch_size]);

            self.datasetQueue.put([in_fb, in_label]);
