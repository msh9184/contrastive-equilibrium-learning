...
from loss.adacos import ADASoftmax
from loss.uniform import Uniformity

class SpeakerNet(nn.Module):

    def __init__(self, max_frames, lr = 0.0001, margin = 1, scale = 1, hard_rank = 0, hard_prob = 0, model="alexnet50", nOut = 512, nSpeakers = 1000, optimizer = 'adam', encoder_type = 'SAP', normalize = True, trainfunc='anglecontrast', trainfunc2='uniform', **kwargs):
        ...
        if trainfunc == 'angleproto':
            ...
            self.__train_normalize__    = False
            self.__test_normalize__     = True
        elif trainfunc == 'adasoftmax':
            self.__L__ = ADASoftmax(in_feats=nOut, n_classes=nSpeakers, m=margin).cuda()
            self.__train_normalize__    = False
            self.__test_normalize__     = True
        ...
        if trainfunc2 == 'uniform':
            self.__U__ = Uniformity().cuda()
            self.uniform = True
    ...
    def train_network(self, loader):
        ...
        for data, data_label in loader:
            ...
            data = torch.stack(data,dim=0).unsqueeze(2)
            ...
            nloss1, prec1 = self.__L__.forward(feat,label)
            
            if self.uniform == True:
                nloss2, _ = self.__U__.forward(feat)

                nloss = nloss1 + nloss2

                loss1 += nloss1.detach().cpu()
                loss2 += nloss2.detach().cpu()
            ...
            if self.uniform == True:
                sys.stdout.write("Loss %f, Loss1 %f, Loss2 %f, TAUC %2.3f%% - %.2f Hz "%(loss/counter, loss1/counter, loss2/counter, top1/counter, stepsize/telapsed));
            else:
                sys.stdout.write("Loss %f TAUC %2.3f%% - %.2f Hz "%(loss/counter, top1/counter, stepsize/telapsed));
    ...
    def evaluateFromListSave(self, listfilename, print_interval=5000, feat_dir='', test_path='', num_eval=10, eval_frames=0):
        ...
        for idx, file in enumerate(setfiles):
            ...
            inp1 = loadWAV(os.path.join(test_path,file), max_frames=eval_frames, evalmode=True, num_eval=num_eval).cuda()
